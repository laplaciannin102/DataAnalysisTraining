...
def get_classes(in_df, columns=None, n_classes=1, drop=False, suffix='_band'):
    ...
    # 階級に分けたものをダミー変数化する
    df2 = pd.get_dummies(df1, columns=list(bins.keys()))
    """
    out_df = in_df.copy()
...
class AnnnPreprocessing():
    ...
    Attributes:
        ...
        ignore_cols: list of str, default=[]
            前処理の対象外となるカラムのリスト。
        
        cls_cols: list of str, default=[]
            階級で分ける対象のカラムのリスト。
        
        cate_cols: list of str, default=[]
            カテゴリー変数のカラムのリスト。
    ...
    def set_df(self, df):
        ...
        # latest_dfがNoneの時はlatest_dfを初期化する。
        if self.latest_df_cols is None:
            self.init_df()
    ...
    def set_ignore_cols(self, cols):
        ...
        """
        if type(cols) == list:
            self.ignore_cols = [ii for ii in cols if ii in self.latest_df_cols]
    ...
    def set_cls_cols(self, cols):
        ...
        """
        if type(cols) == list:
            self.cls_cols = [ii for ii in cols if ii in self.latest_df_cols]
    ...
    def set_cate_cols(self, cols):
        ...
        """
        if type(cols) == list:
            self.cate_cols = [ii for ii in cols if ii in self.latest_df_cols]
    

    def add_pp(self, preprocessing=(lambda x: x), pp_num=None):
        """
        前処理を追加する。

        Args:
            preprocessing: function, optional(default=(lambda x: x))
                DataFrameに前処理を施す関数。
                デフォルトでは恒等写像。
    ...
    def exe_pp(self):
        ...
        # 前処理対象外のカラムを隔離
        ignore_df = df[self.ignore_cols]
        df = df.drop(labels=self.ignore_cols, axis=1)
